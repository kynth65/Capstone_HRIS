...
function Recruitment_Management() {
    ...
    const handleDeletePosition = async (positionId) => {
        console.log("To onboarding with ID:", positionId); // Log the candidate ID for debugging

        // Optimistically update the UI by removing the position before making the request
        ...
        try {
            ...
            // If the request fails, revert the UI by restoring the position
            setPositions(null);
            // Handle any errors during the request and show an error message
    ...
    const handlePreview = async (filename) => {
        try {
            const response = await axios.get(
                ...
                { responseType: "blob" }, // Ensure blob format is used for file response
            );
            ...
            // Open the PDF file in a new tab for viewing
                ...
                {isPdfModalOpen && pdfUrl && (
                    ...
                    >
                        <div className="transparent p-6 rounded-xl w-3/4 xl:w-3/4 h-full text-black overflow-hidden flex flex-col">
                            ...
                            <iframe
                                src={`http://127.0.0.1:5000/uploads/${pdfUrl}`}
