...
openai.api_key = "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"  # Use an environment variable in production
...
def rank():
    ...
    for idx, (resume_idx, percentage, matched_words, comments) in enumerate(ranked_resumes):
        ...
        save_ranking(filename, percentage, position_id, name, email, matched_words, comments)
...
def save_ranking(filename, percentage, position_id, name, email, matched_words, comments):
    position_name = fetch_position_name(position_id)
    
    try:
        query = text("""
            INSERT INTO resume_rankings (filename, percentage, position_id, position_name, name, email, matched_words, comments)
            VALUES (:filename, :percentage, :position_id, :position_name, :name, :email, :matched_words, :comments)
        """)
        db.session.execute(query, {
            ...
            'name': name,
            'email': email,
            'matched_words': matched_words,  # Store matched keywords
            'comments': comments             # Store AI comments
        })
        db.session.commit()
